...
export async function apiFetch<T>(
  ...
  const res = await fetch(url, {
    ...fetchOptions,
    headers: {
      // No forzar Content-Type en uploads de FormData — el browser lo setea
...
export const onboardingApi = {
  estado: () => apiFetch<any>("/api/onboarding/estado"),

  completarPaso: (paso: string) =>
    apiFetch<{ ok: boolean; paso_completado: string }>("/api/onboarding/completar-paso", {
      method: "POST",
      body: JSON.stringify({ paso }),
    }),

  siguientesPasos: () => apiFetch<any>("/api/onboarding/siguientes-pasos"),

  vencimientosSugeridos: () => apiFetch<any[]>("/api/onboarding/vencimientos-sugeridos"),

  confirmarSugeridos: (ids: number[]) =>
    apiFetch<{ confirmados: number }>("/api/onboarding/vencimientos-sugeridos/confirmar", {
      method: "POST",
      body: JSON.stringify(ids),
    }),

  descartarSugerido: (id: number) =>
    apiFetch<{ ok: boolean }>(`/api/onboarding/vencimientos-sugeridos/${id}`, { method: "DELETE" }),

  importarEmpleados: (file: File) => {
    const form = new FormData();
    form.append("file", file);
    return apiFetch<{ importados: number; saltados: number; errores: string[] }>(
      "/api/onboarding/importar-empleados",
  ...
  importarClientes: (file: File) => {
    ...
    return apiFetch<{ importados: number; saltados: number; vencimientos_sugeridos: number; errores: string[] }>(
      "/api/onboarding/importar-clientes",
      { method: "POST", body: form }
    );
  },
};
